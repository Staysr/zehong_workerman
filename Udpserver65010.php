...
$worker = new Worker("udp://0.0.0.0:65010");
...
$worker->onMessage = function($connection, $data)
{
    ...
    if($dataFormatted == false){
        file_put_contents('udp65010.err.log', date('Y-m-d H:i:s', time()).": $data".PHP_EOL, FILE_APPEND);
        return false;
    }
    file_put_contents('udp65010.log', date('Y-m-d H:i:s', time()).": $dataFormatted".PHP_EOL, FILE_APPEND);
    ...
    $mysql->update($sql);
    file_put_contents('udp65010.log', date('Y-m-d H:i:s', time()).": $sql".PHP_EOL, FILE_APPEND);
    //更新设备上次状态
    if($prestatus != $status){
