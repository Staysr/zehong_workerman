...
$worker = new Worker("udp://0.0.0.0:65011");
...
$worker->onMessage = function($connection, $data)
{
    ...
    if($dataFormatted == false){
        file_put_contents('udp65011.err.log', date('Y-m-d H:i:s', time()).": $data".PHP_EOL, FILE_APPEND);
        return false;
    }
    file_put_contents('udp65011.log', date('Y-m-d H:i:s', time()).": $dataFormatted".PHP_EOL, FILE_APPEND);
    ...
    // 屏蔽的设备
    if(in_array($dnum, $mysql->disable_nums)){
        return false;
    }
